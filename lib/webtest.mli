...
module Suite : sig
  ...
  module Sync : sig
    ...
    (** [bracket setup test teardown] generates a {{:#TYPEtest_fun}test_fun}
        which will use [setup] to create state needed for the test, then pass that
        state to [test], and finally will pass that state to [teardown]. *)
  end
  ...
  module Async : sig
    ...
    val run_one : string -> test_fun -> (string -> unit ) -> (outcome -> unit) -> unit
...
(** A zipper implementation based on {{:Webtest.Suite.html#TYPEt}Suite.t}, which
    ...
    {{:Webtest.Utils.html#VALrun}Utils.run} can be used to traverse the test tree and
    report results. *)
module Zipper : sig
  ...
  (** A type representing the path through a {{:Webtest.Suite.html#TYPEt}Webtest.Suite.t} from the
  ...
  (** Convert a {{:Webtest.Suite.html#TYPEt}Suite.t} into a {{:#TYPEt}Zipper.t}. *)
